<template>
  <div id="projectTable" class="app-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" size="mini" style="margin-left: 10px;" type="primary" icon="el-icon-edit" v-if="roles[0] == 'admin'" @click="operate('add')">新增</el-button>
      ...
      <el-table-column align="center" fixed="right" label="操作" width="190">
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-button size="mini" type="primary" @click="auditFun(2,scope.row)" v-show="scope.row.w_statu == '2' && roles[0] == 'admin'">审批</el-button>
            <el-button size="mini" type="primary" @click="auditFun(1,scope.row)" v-show="scope.row.w_statu == '2'">延时</el-button>
            <el-button size="mini" type="primary" @click="auditFun(3,scope.row)" v-show="(scope.row.w_statu == '3' || scope.row.w_statu == '4') && roles[0] == 'admin'">{{scope.row.w_statu == '4'? '修改评分': '评分'}}</el-button>
            <el-button
              size="mini"
              type="primary"
              v-show="(scope.row.w_statu == '1' || scope.row.w_statu == '2') && roles[0] != 'admin'"
            ...
            <el-button
              ...
              @click="operation({ w_id: scope.row.w_id }, '确认删除吗', '/delWork')"
            >删除
            </el-button>
          </el-button-group>
        </template>
    ...
    <el-table key='bTable' v-else-if="listQuery.w_statu == '2' || listQuery.w_statu == '3'" v-loading="listLoading" :data="list" element-loading-text="Loading" border fit>
      ...
      <el-table-column align="center" fixed="right" label="操作" :width="listQuery.w_statu==3? '120': '270'">
        <template slot-scope="scope">
          <el-button-group>
            <el-button size="mini" type="primary" @click="detail(scope.row)">查看</el-button>
            <el-button size="mini" type="primary" @click="operate('edit',scope.row)" v-show="scope.row.w_statu == '1' || scope.row.w_statu == '2'">编辑</el-button>
    ...
    <el-table key='cTable' v-else="listQuery.w_statu == '4'" v-loading="listLoading" :data="list" element-loading-text="Loading" border fit>
      ...
      <el-table-column label="质量评分" :show-overflow-tooltip="true" width="110" prop="w_score_qua" sortable ></el-table-column>
      <el-table-column label="时间评分" :show-overflow-tooltip="true" width="110" prop="w_score_time" sortable ></el-table-column>
      <el-table-column label="总分" :show-overflow-tooltip="true" width="80" prop="w_score_sum" sortable ></el-table-column>
      <el-table-column label="状态" prop="w_statu" :show-overflow-tooltip="true" sortable width="90" align="center">
        <template slot-scope="scope">
          <el-tag :type="scope.row.w_statu | statusFilter">{{ scope.row.w_statu | statuTextFilter }}</el-tag>
        </template>
    ...
    >
      <el-form ref="formAdd" :model="form" label-position="right" label-width="105px">
        ...
        >
          <el-input v-model="form.w_title" placeholder="请输入任务名称" />
        </el-form-item>
        <el-form-item
          label="任务内容："
          prop="w_content"
        ...
        >
          <el-input :rows="4" type="textarea" v-model="form.w_content" placeholder="请输入任务内容" />
        </el-form-item>
        <el-form-item
          label="任务来源："
          prop="w_source"
        ...
        >
          <el-input v-model="form.w_source" placeholder="请输入任务来源" />
        </el-form-item>
        <el-form-item
          label="责任部门："
          prop="dep"
        ...
        >
          <el-select v-model="form.dep" placeholder="请选择责任部门" style="width:100%;">
            <el-option
        ...
        >
          <el-select v-model="form.userid" placeholder="请选择责任人" style="width:100%;">
            <el-option
...
<script>
  ...
  import { getWorkList, addWork, editWork, delayWork, auditWork, finishWork, scoreWork } from '@/api/work'
  ...
  export default {
    ...
    methods: {
      ...
      fetchData() {
        ...
        getWorkList(this.listQuery).then(response => {
          if (response) {
