...
// 获取 全部任务列表
router.get('/getWorkList', checkToken, async (ctx, next) => {
  var count;
  let user = ctx.userInfo, dep = '', nameid = '';
  let title= ctx.query.w_title
  let w_userid= ctx.query.w_userid
  let statu= ctx.query.w_statu
  let starttime= ctx.query.w_starttime
  let endtime=ctx.query.w_endtime
  let page= ctx.query.page - 1 || 0
  let size= ctx.query.size || 10
  if (user.roles == 'editor') {
    dep= user.dep
  } else if (user.roles == 'other') {
    nameid = user.userid
  }
  ...
  await postsModel.getWorkList(title, w_userid, dep, nameid, statu, starttime, endtime, page, size).then((res) => {
    ...
    let pageArr =result.slice(page*size, (page+1)*size)
    ctx.body = {
      state: 1,
      msg: '任务列表获取成功',
      data: {
